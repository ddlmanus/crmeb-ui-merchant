  ...
  methods: {
    ...
    setLocalTheme(val) {
      ...
      if (['theme-1', 'theme-2'].includes(val)) {
        this.$store.state.themeConfig.themeConfig.primary = '#4073FA'; //蓝黑蓝白
        ...
        this.$store.state.themeConfig.themeConfig.primary = '#4073FA'; //默认蓝
      }
      ...
      if (['classic'].includes(this.getThemeConfig.layout)) {
        ...
        if (val == 'theme-1') {
          ...
          themeSelect['--prev-color-primary'] = '#4073FA';
          themeSelect['--prev-MenuActiveColor'] = '#4073FA';
        } else if (val == 'theme-3') {
    ...
    setLocalThemeConfigStyle() {
      Local.set('JavaMerThemeConfigStyle', document.documentElement.style.cssText);
    },
    // 布局配置弹窗打开
    openDrawer() {
